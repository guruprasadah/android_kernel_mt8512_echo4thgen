...
{
	u32 wdt_boot_reason = 0;
	int ret;
	*reason = lc_boot;

	ret = mtk_read_nonrst_reg2(&wdt_boot_reason);
	...
	wdt_boot_reason &= LIFE_CYCLE_WARMBOOT_REASON_BIT_MASK;

	pr_info("%s: life cycle boot reason is 0x%x\n", __func__,
							wdt_boot_reason);

	if (lc_boot == LIFE_CYCLE_NOT_AVAILABLE) {
		switch (wdt_boot_reason) {
		case LIFE_CYCLE_WARMBOOT_REASON_KERNEL_PANIC_BITS_VALUE:
			*reason = WARMBOOT_BY_KERNEL_PANIC;
			break;
		case LIFE_CYCLE_WARMBOOT_REASON_KERNEL_WDOG_BITS_VALUE:
			*reason = WARMBOOT_BY_KERNEL_WATCHDOG;
			break;
		case LIFE_CYCLE_WARMBOOT_REASON_HW_WDOG_BITS_VALUE:
			*reason = WARMBOOT_BY_HW_WATCHDOG;
			break;
		case LIFE_CYCLE_WARMBOOT_REASON_SW_BITS_VALUE:
			*reason = WARMBOOT_BY_SW;
			break;
		default:
...
static int mtk_write_boot_reason(enum life_cycle_reason reason)
{
	...
	case WARMBOOT_BY_KERNEL_PANIC:
		wdt_boot_reason =
				LIFE_CYCLE_WARMBOOT_REASON_KERNEL_PANIC_BITS_VALUE;
		break;
	case WARMBOOT_BY_KERNEL_WATCHDOG:
		wdt_boot_reason =
				LIFE_CYCLE_WARMBOOT_REASON_KERNEL_WDOG_BITS_VALUE;
		break;
	case WARMBOOT_BY_HW_WATCHDOG:
		wdt_boot_reason =
				LIFE_CYCLE_WARMBOOT_REASON_HW_WDOG_BITS_VALUE;
		break;
	case WARMBOOT_BY_SW:
		wdt_boot_reason =
				LIFE_CYCLE_WARMBOOT_REASON_SW_BITS_VALUE;
		break;
	default:
		pr_err("%s: unsupported life cycle boot reason: 0x%x\n",
							__func__, reason);
		return -EOPNOTSUPP;
	}

	ret = mtk_setbits_nonrst_reg2(LIFE_CYCLE_WARMBOOT_REASON_BIT_MASK,
							wdt_boot_reason);
	if (ret < 0) {
...
static int mtk_read_thermal_shutdown_reason(enum life_cycle_reason *reason)
{
	...
	wdt_shutdown_reason &= LIFE_CYCLE_THERMAL_SHUTDOWN_REASON_BIT_MASK;

	pr_info("%s: thermal life cycle reason is 0x%x\n", __func__,
							wdt_shutdown_reason);

	switch (wdt_shutdown_reason) {
	case LIFE_CYCLE_THERMAL_SHUTDOWN_REASON_SOC_BITS_VALUE:
		*reason = THERMAL_SHUTDOWN_REASON_SOC;
		break;
	case LIFE_CYCLE_THERMAL_SHUTDOWN_REASON_SVS_BITS_VALUE:
		*reason = THERMAL_SHUTDOWN_REASON_SVS;
		break;
	case LIFE_CYCLE_THERMAL_SHUTDOWN_REASON_CVS_BITS_VALUE:
		*reason = THERMAL_SHUTDOWN_REASON_CVS;
		break;
	case LIFE_CYCLE_THERMAL_SHUTDOWN_REASON_CPVS_BITS_VALUE:
		*reason = THERMAL_SHUTDOWN_REASON_CPVS;
		break;
	case LIFE_CYCLE_THERMAL_SHUTDOWN_REASON_BVS_BITS_VALUE:
		*reason = THERMAL_SHUTDOWN_REASON_BVS;
		break;
	case LIFE_CYCLE_THERMAL_SHUTDOWN_REASON_B1VS_BITS_VALUE:
		*reason = THERMAL_SHUTDOWN_REASON_B1VS;
		break;
	case LIFE_CYCLE_THERMAL_SHUTDOWN_REASON_C1VS_BITS_VALUE:
		*reason = THERMAL_SHUTDOWN_REASON_C1VS;
		break;
	default:
		*reason = LIFE_CYCLE_NOT_AVAILABLE;
	}
...
static int mtk_write_thermal_shutdown_reason(enum life_cycle_reason reason)
{
	...
	case THERMAL_SHUTDOWN_REASON_SOC:
		wdt_shutdown_reason =
				LIFE_CYCLE_THERMAL_SHUTDOWN_REASON_SOC_BITS_VALUE;
		break;
	case THERMAL_SHUTDOWN_REASON_SVS:
		wdt_shutdown_reason =
				LIFE_CYCLE_THERMAL_SHUTDOWN_REASON_SVS_BITS_VALUE;
		break;
	case THERMAL_SHUTDOWN_REASON_CVS:
		wdt_shutdown_reason =
				LIFE_CYCLE_THERMAL_SHUTDOWN_REASON_CVS_BITS_VALUE;
		break;
	case THERMAL_SHUTDOWN_REASON_CPVS:
		wdt_shutdown_reason =
				LIFE_CYCLE_THERMAL_SHUTDOWN_REASON_CPVS_BITS_VALUE;
		break;
	case THERMAL_SHUTDOWN_REASON_BVS:
		wdt_shutdown_reason =
				LIFE_CYCLE_THERMAL_SHUTDOWN_REASON_BVS_BITS_VALUE;
		break;
	case THERMAL_SHUTDOWN_REASON_B1VS:
		wdt_shutdown_reason =
				LIFE_CYCLE_THERMAL_SHUTDOWN_REASON_B1VS_BITS_VALUE;
		break;
	case THERMAL_SHUTDOWN_REASON_C1VS:
		wdt_shutdown_reason =
				LIFE_CYCLE_THERMAL_SHUTDOWN_REASON_C1VS_BITS_VALUE;
		break;
	default:
	...
	ret = mtk_setbits_nonrst_reg2(LIFE_CYCLE_THERMAL_SHUTDOWN_REASON_BIT_MASK,
							wdt_shutdown_reason);
	if (ret < 0) {
		pr_err("%s: error(%d) writing nonrst reg2 from wdt\n",
							__func__, ret);
...
static int mtk_lcr_reset(void)
{
	...
	mtk_setbits_nonrst_reg2(LIFE_CYCLE_THERMAL_SHUTDOWN_REASON_BIT_MASK, 0);
	mtk_setbits_nonrst_reg2(LIFE_CYCLE_WARMBOOT_REASON_BIT_MASK, 0);
	return 0;
}
