...
static void mtk_i2c_init_hw(struct mtk_i2c *i2c)
{
	...
	if (i2c->dev_comp->ltiming_adjust)
		writew(i2c->ltiming_reg, i2c->base + OFFSET_LTIMING);

	if (i2c->dev_comp->timing_adjust) {
		writew(i2c->ac_timing.ext, i2c->base + OFFSET_EXT_CONF);
		writew(i2c->ac_timing.inter_clk_div, i2c->base + OFFSET_CLOCK_DIV);
		writew(I2C_SCL_MIS_COMP_VALUE, i2c->base + OFFSET_SCL_MIS_COMP_POINT);
		writew(i2c->ac_timing.sda_timing, i2c->base + OFFSET_SDA_TIMING);

		if (i2c->dev_comp->ltiming_adjust) {
			writew(i2c->ac_timing.htiming, i2c->base + OFFSET_TIMING);
			writew(i2c->ac_timing.hs, i2c->base + OFFSET_HS);
			writew(i2c->ac_timing.ltiming, i2c->base + OFFSET_LTIMING);
		} else {
			writew(i2c->ac_timing.scl_hl_ratio, i2c->base + OFFSET_SCL_HIGH_LOW_RATIO);
			writew(i2c->ac_timing.hs_scl_hl_ratio, i2c->base + OFFSET_HS_SCL_HIGH_LOW_RATIO);
			writew(i2c->ac_timing.sta_stop, i2c->base + OFFSET_STA_STO_AC_TIMING);
			writew(i2c->ac_timing.hs_sta_stop, i2c->base + OFFSET_HS_STA_STO_AC_TIMING);
		}
	}
				   ...
				   unsigned int sample_cnt)
{
	...
	unsigned long sample_ns = (1000000000 * (sample_cnt + 1)) / clk_src;
...
static void i2c_dump_info(struct mtk_i2c *i2c)
{
	...
	dev_err(i2c->dev,"I2C base address %p\n", i2c->base);
	for (reg_off = 0; reg_off < 0xA0; reg_off += 0x10) {
		dev_err(i2c->dev,"addr: 0x%x: %x-%x-%x-%x \n",
			(reg_off),
			i2c_readw(i2c, reg_off),
			i2c_readw(i2c, (reg_off + 0x04)),
			i2c_readw(i2c, (reg_off + 0x08)),
			i2c_readw(i2c, (reg_off + 0x0c)));
	}
	dev_err(i2c->dev,"I2C dma base address %p\n", i2c->pdmabase);
	for (reg_off = 0; reg_off < 0x80; reg_off += 0x10) {
		dev_err(i2c->dev,"addr: 0x%x: %x-%x-%x-%x \n",
			(reg_off),
			readw(i2c->pdmabase + reg_off),
			readw(i2c->pdmabase + reg_off + 0x04),
			readw(i2c->pdmabase + reg_off + 0x08),
			readw(i2c->pdmabase + reg_off + 0x0c));
	}
}
