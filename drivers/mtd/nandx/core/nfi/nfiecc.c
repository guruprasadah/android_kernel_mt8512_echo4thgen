...
static void  dump_ecc_regs(void *reg_base)
{
	u32 *addr =  (u32 *)reg_base;
	u32 i;

	for (i = 0; i< 0x200; i++) {
		pr_debug("nfiecc reg addr:0x%x, value:0x%x\n", i * 4, readl(addr));
		addr++;
	}
}
...
static int nfiecc_correct_data(struct nfiecc *ecc, u8 *data, u32 sector)
{
	u32 err, offset, i;
	u32 loc, byteloc, bitloc;

	ecc->ecc_status.corrected = 0;
	ecc->ecc_status.failed = 0;
	...
	if (err == ecc->caps->err_mask) {
		ecc->ecc_status.failed++;
		return -ENANDREAD;
	}

	ecc->ecc_status.corrected += err;
	ecc->ecc_status.bitflips = max_t(u32, ecc->ecc_status.bitflips, err);
				...
				u32 sectors)
{
	...
	ecc->ecc_status.bitflips = bitflips;
