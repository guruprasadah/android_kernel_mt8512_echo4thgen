...
static int nfi_ctrl(struct nfi *nfi, int cmd, void *args)
{
	...
	case NFI_CTRL_ECC_GET_ECC_INFO:
		*(struct nfiecc **)args = nb->ecc;
		break;
	...
	if (nb->ecc_en) {
		for (i = 0; i < sectors; i++) {
			buf = nb->buf + i * (nfi->sector_size +
					     nfi->sector_spare_size);
			ret = nb->ecc->correct_data(nb->ecc, buf, i);
			if (data)
	...
	if (nb->dma_en) {
		if (read) {
			...
			if (nb->read_status == -ENANDREAD) {
				...
				if (is_empty) {
					nb->read_status = 0;
					pr_err("%s: empty page checked here.#%d\n", __func__, nb->row);

					/* try to memset 0xff for empty page */
					memset(data, 0xff, data_len);
					memset(fdm, 0xff, nb->access_len - data_len);
				}
			}
		} else {
			nandx_dma_unmap(nb->res.dev, nb->buf, nb->dma_addr,
