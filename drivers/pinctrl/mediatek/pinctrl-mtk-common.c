...
#ifdef CONFIG_MTK_HIFI4DSP_SUPPORT
	else if (!strncmp(buf, "bspmode", strlen("bspmode"))
		&& (sscanf(buf+7, "%d %d", &pin, &val) == 2)) {
		val_set = gpio_send_ipi_msg(pin, SET_MODE_IPI, val);
	} else if (!strncmp(buf, "bspdir", strlen("bspdir"))
		&& (sscanf(buf+6, "%d %d", &pin, &val) == 2)) {
		val_set = gpio_send_ipi_msg(pin, SET_DIR_IPI, val);
	} else if (!strncmp(buf, "bspout", strlen("bspout"))
		&& (sscanf(buf+6, "%d %d", &pin, &val) == 2)) {
		val_set = gpio_send_ipi_msg(pin, SET_DOUT_IPI, val);
	} else if (!strncmp(buf, "bsppullen", strlen("bsppullen"))
		&& (sscanf(buf+9, "%d %d", &pin, &val) == 2)) {
		val_set = gpio_send_ipi_msg(pin, SET_PULLEN_IPI, val);
	} else if ((!strncmp(buf, "bsppullsel", strlen("bsppullsel")))
		&& (sscanf(buf+10, "%d %d", &pin, &val) == 2)) {
		val_set = gpio_send_ipi_msg(pin, SET_PULLSEL_IPI, val);
	} else if ((!strncmp(buf, "bspies", strlen("bspies")))
		&& (sscanf(buf+6, "%d %d", &pin, &val) == 2)) {
		val_set = gpio_send_ipi_msg(pin, SET_IES_IPI, val);
	} else if ((!strncmp(buf, "bspsmt", strlen("bspsmt")))
		&& (sscanf(buf+6, "%d %d", &pin, &val) == 2)) {
		val_set = gpio_send_ipi_msg(pin, SET_SMT_IPI, val);
	}
#endif
	return count;
}
