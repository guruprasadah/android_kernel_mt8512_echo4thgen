...
static void mtk_spi_slave_ready(struct mtk_spi_slave *mdata, bool ready)
{
	int value, retries = 0;
	static unsigned int gpio_fixup_count;
	const int retries_max = 2000;

	if (!gpio_is_valid(mdata->slave_ready_gpio))
		return;

	do {
		gpio_set_value(mdata->slave_ready_gpio, ready);
		value = gpio_get_value(mdata->slave_ready_gpio);

		dev_dbg(mdata->dev, "slave ready=%d gpio=%d value wrote=%d retries=%d total_fixups=%d\n",
			ready, mdata->slave_ready_gpio,
			gpio_get_value(mdata->slave_ready_gpio),
			retries, gpio_fixup_count);
	...
	if (retries > 0) {
		if (ready == value) {
			gpio_fixup_count++;
			dev_info_ratelimited(mdata->dev, "fixed up gpio_set_value(), wrote=%d retries=%d, total_fixups=%d\n",
					     ready, retries, gpio_fixup_count);
		} else {
			dev_err(mdata->dev, "failed to set gpio retries=%d wrote=%d read=%d\n",
				retries, ready, value);
		}
	}
}
...
static int mtk_spi_slave_wait_for_completion(struct mtk_spi_slave *mdata)
{
	...
	mtk_spi_slave_ready(mdata, true);

	if (wait_for_completion_interruptible(&mdata->xfer_done) ||
	    mdata->slave_aborted) {
		dev_dbg(mdata->dev, "interrupted\n");
		ret = -EINTR;
	}

	mtk_spi_slave_ready(mdata, false);
...
static int mtk_spi_slave_probe(struct platform_device *pdev)
{
	...
	ctlr->rt = of_property_read_bool(pdev->dev.of_node, "task-rt");
	...
	mdata->slave_ready_gpio = of_get_named_gpio(pdev->dev.of_node,
	                                            "slave-ready-gpio", 0);
	if (!gpio_is_valid(mdata->slave_ready_gpio)) {
		dev_warn(&pdev->dev, "invalid slave_read_gpio %d\n",
		         mdata->slave_ready_gpio);
	} else {
		ret = devm_gpio_request(&pdev->dev, mdata->slave_ready_gpio,
		                        "slave-ready");
		if (ret) {
			dev_err(&pdev->dev,
			        "slave-ready gpio request failed %d\n", ret);
			goto err_put_ctlr;
		}

		gpio_direction_output(mdata->slave_ready_gpio, 1);
		mtk_spi_slave_ready(mdata, false);
	}
