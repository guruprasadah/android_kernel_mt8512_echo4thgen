...
static int st_lis2dw12_update_fifo(struct iio_dev *iio_dev, bool enable)
{
	...
	if (enable) {
		hw->ts_fifo_irq = hw->ts = st_lis2dw12_get_timestamp();
...
static int st_lis2dw12_read_fifo(struct st_lis2dw12_hw *hw)
{
	...
	if (status & ST_LIS2DW12_FIFO_SAMPLES_OVR_MASK) {
		dev_info(hw->dev, "<< Accel FIFO overun >> : 0x%x\n", status);
	}

	samples = status & ST_LIS2DW12_FIFO_SAMPLES_DIFF_MASK;
	delta_ts = div_s64(hw->delta_ts, hw->watermark);
	if (samples > hw->watermark)
		samples = hw->watermark;
	...
	while (read_len < fifo_len) {
		...
		for (i = 0; i < word_len; i += ST_LIS2DW12_DATA_SIZE) {
			...
			hw->ts = min_t(s64, hw->ts_fifo_irq, hw->ts);
			       ...
			       const char *buf, size_t size)
{
	...
	hw->ts_fifo_irq = st_lis2dw12_get_timestamp();
	...
	code = IIO_UNMOD_EVENT_CODE(IIO_ACCEL, -1,
				    ...
				    IIO_EV_DIR_EITHER);
	iio_push_event(iio_dev, code, hw->ts_fifo_irq);
...
static irqreturn_t st_lis2dw12_handler_thread(int irq, void *private)
{
	...
	if (status & ST_LIS2DW12_STATUS_FTH_MASK) {
		mutex_lock(&hw->fifo_lock);
		hw->delta_ts = st_lis2dw12_ewma(hw->delta_ts, hw->ts_irq - hw->ts_fifo_irq,
					ST_LIS2DW12_EWMA_LEVEL);
		if (hw->ts < hw->ts_fifo_irq)
			hw->ts = hw->ts_fifo_irq;
		hw->ts_fifo_irq = hw->ts_irq;
	...
	if (((status & ST_LIS2DW12_STATUS_TAP_MASK) &&
		...
		if (source & ST_LIS2DW12_DTAP_SRC_MASK) {
			...
			dev_dbg(hw->dev, "%s 0x%x 0x%x\n", "<< Double TAP >>", status, source);
		}
...
int st_lis2dw12_fifo_setup(struct st_lis2dw12_hw *hw)
{
	...
	u32 irq_type = irqd_get_trigger_type(irq_get_irq_data(hw->irq));

	irq_flags = irq_type & IRQF_TRIGGER_MASK;
