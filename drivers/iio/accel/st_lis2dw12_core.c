...
#define ST_LIS2DW12_INT_MASK			GENMASK(5, 3)
#define ST_LIS2DW12_SLPMODESEL_MASK		BIT(1)

#define ST_LIS2DW12_CTRL4_INT1_CTRL_ADDR	0x23
#define ST_LIS2DW12_DRDY_MASK			BIT(0)
#define ST_LIS2DW12_FTH_INT_MASK		BIT(1)
#define ST_LIS2DW12_FIFO_FUL_MASK		BIT(2)
...
#define ST_LIS2DW12_FDS_MASK			BIT(3)
...
#define ST_LIS2DW12_6D_THS_MASK			GENMASK(6, 5)
#define ST_LIS2DW12_PRIOR_MASK			GENMASK(7, 5)
...
#define ST_LIS2DW12_X_OFS_USR_ADDR		0x3c
#define ST_LIS2DW12_Y_OFS_USR_ADDR		0x3d
#define ST_LIS2DW12_Z_OFS_USR_ADDR		0x3e
#define ST_LIS2DW12_OFS_USR_MASK		0xff

#define ST_LIS2DW12_ABS_INT_CFG_ADDR		0x3f
#define ST_LIS2DW12_USR_OFF_W_MASK		BIT(2)
#define ST_LIS2DW12_USR_OFF_ON_OUT_MASK		BIT(4)
...
#define DEFAULT_FIFO_WATERMARK			0x19
#define DEFAULT_LP_MODE				0x3
...
static int st_lis2dw12_set_odr(struct st_lis2dw12_sensor *sensor, u16 req_odr)
{
	...
	val = (st_lis2dw12_odr_table[i].val << __ffs(ST_LIS2DW12_ODR_MASK)) |
	      (mode << __ffs(ST_LIS2DW12_MODE_MASK)) | DEFAULT_LP_MODE;
...
static int st_lis2dw12_init_hw(struct st_lis2dw12_hw *hw)
{
	...
	if (irq_type & (IRQ_TYPE_LEVEL_LOW | IRQ_TYPE_EDGE_FALLING))
		irq_conf = 0x7; /* opendrain, latched, activelow */
	else
		irq_conf = 0x6; /* opendrain, latched, activehigh */

	/* configure calibration data*/
	if (hw->calib_data) {
		/* hw->calib_data = 0x00XXYYZZ */
		err = st_lis2dw12_write_with_mask(hw,
			ST_LIS2DW12_X_OFS_USR_ADDR, ST_LIS2DW12_OFS_USR_MASK,
			hw->calib_data >> 16);
		if (err < 0)
			return err;

		err = st_lis2dw12_write_with_mask(hw,
			ST_LIS2DW12_Y_OFS_USR_ADDR, ST_LIS2DW12_OFS_USR_MASK,
			hw->calib_data >> 8);
		if (err < 0)
			return err;

		err = st_lis2dw12_write_with_mask(hw,
			ST_LIS2DW12_Z_OFS_USR_ADDR, ST_LIS2DW12_OFS_USR_MASK,
			hw->calib_data);
		if (err < 0)
			return err;
	}
	...
	err = st_lis2dw12_write_with_mask(hw, ST_LIS2DW12_CTRL3_ADDR,
			ST_LIS2DW12_INT_MASK | ST_LIS2DW12_SLPMODESEL_MASK,
			irq_conf << 2 | 1);
	...
	/* configure interrupts and user offset */
	err = st_lis2dw12_write_with_mask(hw, ST_LIS2DW12_ABS_INT_CFG_ADDR,
			ST_LIS2DW12_ALL_INT_MASK | ST_LIS2DW12_USR_OFF_ON_OUT_MASK
			| ST_LIS2DW12_USR_OFF_W_MASK, 0xc);
	if (err < 0)
		return err;
	...
	err = st_lis2dw12_write_with_mask(hw, ST_LIS2DW12_TAP_THS_X_ADDR,
					  ST_LIS2DW12_6D_THS_MASK, 3);
	if (err < 0)
		return err;

	err = st_lis2dw12_write_with_mask(hw, ST_LIS2DW12_TAP_THS_Y_ADDR,
					  ST_LIS2DW12_PRIOR_MASK, 7);
	if (err < 0)
		return err;

	/* high-pass filter enabled by default */
	err = st_lis2dw12_write_with_mask(hw, ST_LIS2DW12_CTRL6_ADDR,
					  ST_LIS2DW12_FDS_MASK, 1);
	if (err < 0)
		return err;

	/* BW = ODR/20 */
	err = st_lis2dw12_write_with_mask(hw, ST_LIS2DW12_CTRL6_ADDR,
					  ST_LIS2DW12_BW_MASK, 0x3);
	if (err < 0)
		return err;
	...
	return st_lis2dw12_write_with_mask(hw, drdy_reg,
			ST_LIS2DW12_FTH_INT_MASK | ST_LIS2DW12_FIFO_FUL_MASK,
			0x3);
}
...
static int st_lis2dw12_parse_dt(struct st_lis2dw12_hw *hw)
{
	int ret;
	const char *calib_data;
	struct device_node *node = hw->dev->of_node;

	hw->wakeup = of_property_read_bool(node, "wakeup-source");

	/* Read calibration data */
	ret = of_property_read_string(node, "st,calibdata",
					&calib_data);
	if (ret) {
		dev_info(hw->dev, "calibaration data not found in DT\n");
	} else {
		ret = sscanf(calib_data, "%x", &hw->calib_data);
		if (ret < 1)
			hw->calib_data = 0;
	}
		      ...
		      const struct st_lis2dw12_transfer_function *tf_ops)
{
	...
	hw->irq = irq;
	hw->tf = tf_ops;
	hw->watermark = DEFAULT_FIFO_WATERMARK;

	err = st_lis2dw12_parse_dt(hw);
	if (err < 0)
		dev_err(hw->dev, "Failed to parse DT : %d\n", err);
