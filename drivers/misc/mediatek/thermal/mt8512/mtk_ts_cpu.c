...
/**
 * The first two trip temps are the critical HW protection points defined by our
 * thermal HW engineers. These are +5C to the critical SoC temp defined in the
 * Amazon thermal policy.
 */
static int trip_temp[MAX_TRIPS] = {102000, 102000, 85000, 75000, 65000,
...
static bool resume_temp_update_pending;
...
static int tscpu_get_temp(struct thermal_zone_device *thermal, int *t)
{
	...
	if (resume_temp_update_pending) {
		msleep(50);
		read_all_temperature();
		resume_temp_update_pending = false;
	}
...
static int tscpu_thermal_resume(struct platform_device *dev)
{
	...
	resume_temp_update_pending = true;
