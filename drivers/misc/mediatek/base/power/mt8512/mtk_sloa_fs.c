				 ...
				 size_t count, loff_t *f_pos)
{
	...
	len += snprintf(buf + len, BUF_SIZE - len,
			"infrn0_mask: echo infra0_mask 0xx > %s\n", d);
	len += snprintf(buf + len, BUF_SIZE - len,
			"infra2_mask: echo infra2_mask 0xx > %s\n", d);
	#if 0
	len += snprintf(buf + len, BUF_SIZE - len,
			"infra3_mask: echo infra3_mask 0xx > %s\n", d);
	len += snprintf(buf + len, BUF_SIZE - len,
			"infra4_mask: echo infra4_mask 0xx > %s\n", d);
	len += snprintf(buf + len, BUF_SIZE - len,
			"mmsys0_mask: echo mmsys0_mask 0xx > %s\n", d);
	len += snprintf(buf + len, BUF_SIZE - len,
			"mmsys_oth_mask: echo mmsys_oth_mask 0xx > %s\n", d);
	len += snprintf(buf + len, BUF_SIZE - len,
			"image0_mask: echo image0_mask 0xx > %s\n", d);
	len += snprintf(buf + len, BUF_SIZE - len,
			"image1_mask: echo image1_mask 0xx > %s\n", d);
	len += snprintf(buf + len, BUF_SIZE - len,
			"image_oth_mask: echo image_oth_mask 0xx > %s\n", d);
	#endif
	return simple_read_from_buffer(userbuf, count, f_pos, buf, len);
}
