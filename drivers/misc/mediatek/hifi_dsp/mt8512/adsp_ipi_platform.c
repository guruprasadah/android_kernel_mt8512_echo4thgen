...
unsigned int update_adsp_commmon_info(enum adsp_core_id core_id)
{
	if (core_id >= ADSP_CORE_TOTAL) {
		pr_err("core_id = %d,core number overflow!\n", core_id);
		return 0;
	}
 ...
 */
static struct task_struct *ipi_kthread;
static DECLARE_WAIT_QUEUE_HEAD(ipi_work_wait);
struct completion ipi_work_completion;

int adsp_ipi_work_handler(void *unused)
{
	...
	while (!kthread_should_stop()) {
		wait_for_completion(&ipi_work_completion);

		adsp_ipi_handler(ADSP_CORE_0_ID);

		/* release wakelock */
		adsp_set_wakelock(false);
	}
...
{
	/* hold wakelock on first message out of suspend */
	if (is_from_suspend) {
		adsp_set_wakelock(true);
		is_from_suspend = 0;
	}

	complete(&ipi_work_completion);
...
int adsp_ipi_platform_init(struct platform_device *pdev)
{
	...
	init_completion(&ipi_work_completion);
	ipi_kthread = kthread_run(adsp_ipi_work_handler, NULL, "ipi_kthread");
