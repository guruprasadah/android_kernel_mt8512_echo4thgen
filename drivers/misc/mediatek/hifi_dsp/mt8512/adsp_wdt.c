...
{
	pr_err("[%s] ADSP happens exception!\n", __func__);

	/* panic on DSP watchdog timeout */
	BUG();
...
void adsp_wdt_stop(void)
{
	...
	pr_notice("%s before stop, wdt mode:0x%x\n", __func__, readl(DSP_WDT_MODE));
	...
	pr_notice("%s after stop, wdt mode:0x%x\n", __func__, readl(DSP_WDT_MODE));
}
...
int adsp_wdt_device_init(struct platform_device *pdev)
{
	...
	/* Make sure watchdog is off until DSP is ready */
	adsp_wdt_stop();
