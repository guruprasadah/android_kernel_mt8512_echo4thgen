...
static int es7210_suspend(struct snd_soc_codec *codec)
{
	...
	if (es7210->reset_adc_regulator) {
		ret = regulator_disable(es7210->reset_adc_regulator);
		if (ret) {
			pr_err("%s: ES7210 failed to disable adc regulator(%d)\n", __FUNCTION__, ret);
		}
	}
...
static int es7210_resume(struct snd_soc_codec *codec)
{
	...
	if (es7210->reset_adc_regulator) {
		ret = regulator_enable(es7210->reset_adc_regulator);
		if (ret) {
			pr_err("%s: ES7210 failed to enable adc regulator(%d)\n", __FUNCTION__, ret);
		}
	}
	...
	es7210_multi_chips_update_bits(0x01, 0x79, 0x39);
...
static int es7210_probe(struct snd_soc_codec *codec)
{
	...
	es7210->reset_adc_regulator = devm_regulator_get(&es7210->i2c->dev,
						"reset-adc-regulator");
	if (IS_ERR(es7210->reset_adc_regulator)) {
		pr_err("%s: ES7210 failed to get adc regulator\n", __FUNCTION__);
		es7210->reset_adc_regulator = NULL;
	} else {
		ret = regulator_enable(es7210->reset_adc_regulator);
		if (ret) {
			pr_err("%s: ES7210 failed to regulator_enable(%d)\n", __FUNCTION__, ret);
			return ret;
		}
	}
	/*
...
static int es7210_remove(struct snd_soc_codec *codec)
{
	...
	if (es7210->reset_adc_regulator) {
		regulator_disable(es7210->reset_adc_regulator);
		if (ret) {
			pr_err("%s: ES7210 failed to disable adc regulator(%d)\n", __FUNCTION__, ret);
		}
	}
